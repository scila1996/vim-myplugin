...
def Cpp_run(flag = 0):
    ...
    if flag:
        if not os.path.isdir(build_path):
            os.mkdir(build_path)
        compiler = 'g++ -o {:s}/{:s} {:s}'.format(build_path, current_file_split[0], current_file)
...
nnoremap <buffer> <F5> :update <bar> python Cpp_run() <cr>
nnoremap <buffer> <F7> :update <bar> python Cpp_run(1) <cr>
nnoremap <buffer> <F9> :update <bar> python Cpp_run(2) <cr>
